...
const objLat = (obj) => {
    ...
    obj.firstName=obj.firstName[0].toUpperCase()+obj.firstName.slice(1,obj.firstName.length);
    obj.lastName=obj.lastName[0].toUpperCase()+obj.lastName.slice(1,obj.lastName.length);
...
for (let i=0;i<arr.length;i++){
    if ( arr[i].yearsOfExperience>1){
        let obj={};
        if(arr[i].lastName!==null) obj.fullName=arr[i].firstName+" "+arr[i].lastName
...
const applicationsStatics = (arr) => {
    ...
    let result = {
        ...
        for(let i=0;i<arr.length;i++){
            if(arr[i].firstName===null||""||arr[i].firstName===null||""||arr[i].yearsOfExperience<=1) result.rejectedApplicants++;
            switch(arr[i].tech){
                case "JS":
    ...
    for (let i=0;i<data.grades.length;i++){
        for(let j=0;j<data.grades[i].numberOFClasses;j++){
            ...
            for(let k=0;k<data.grades[i].classes[j].classScores.length;k++){
                avg+=Math.floor(data.grades[i].classes[j].classScores[k]);
                console.log(data.grades[i].classes[j].classScores[k])
            }
            data.grades[i].classes[j].avg=avg/data.grades[i].classes[j].classScores.length;
        }
    }
    return data
};
